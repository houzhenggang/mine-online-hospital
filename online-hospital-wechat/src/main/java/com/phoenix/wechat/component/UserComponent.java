...
import com.phoenix.wechat.model.user.Data;
import com.phoenix.wechat.model.user.Followers;
import com.phoenix.wechat.model.user.User;
...
public class UserComponent extends AbstractComponent {
	...
	public void updateRemark(String openid, String remark) throws WeixinException {
		...
		// 调用获取access_token接口
		Response res = http.post("https://api.weixin.qq.com/cgi-bin/user/info/updateremark?access_token="
				+ weixin.getToken().getAccess_token(), postParam);
		// 根据请求结果判定，是否验证成功
	...
	public List<User> batchGet(String[] openids, String[] langs) throws WeixinException {
		...
		HttpsClient http = new HttpsClient();
		// 调用获取access_token接口
		Response res = http.post("https://api.weixin.qq.com/cgi-bin/user/info/batchget?access_token="
				+ weixin.getToken().getAccess_token(), postUserList);
		// 根据请求结果判定，是否验证成功
		JSONObject jsonObj = res.asJSONObject();
		if (jsonObj == null) {
			return null;
	 ...
	 * @param next_openid 第一个拉取的OPENID，不填默认从头开始拉取
	 * @return 关注者对象
	 * @throws WeixinException 微信操作异常 when Weixin service
	 *         or network is unavailable, or the user has not authorized
	 */
