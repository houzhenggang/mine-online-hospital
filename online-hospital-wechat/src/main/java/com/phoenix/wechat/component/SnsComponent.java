...
import com.phoenix.wechat.model.sns.SnsAccessToken;
import com.phoenix.wechat.model.sns.SnsUser;
...
public class SnsComponent extends AbstractComponent {
	...
	public String getOAuth2CodeUrl(String redirect_uri, String scope, String state) {
		try {
			return authorize_url + "?appid=" + weixin.getAppId() + "&redirect_uri="
					+ URLEncoder.encode(redirect_uri, "UTF-8") + "&response_type=code&scope=" + scope + "&state="
	...
	public SnsAccessToken getSnsOAuth2AccessToken(String code) throws WeixinException {
		...
		// 拼接参数
		String param = "?appid=" + weixin.getAppId() + "&secret=" + weixin.getSecret() + "&code=" + code
